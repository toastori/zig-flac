...
pub fn MultiChannelIter(SampleT: type) type {
    return struct {
        ...
        pub fn init(allocator: std.mem.Allocator, channels_count: u8) !@This() {
            ...
            const big_samples: []SampleT = try allocator.alloc(SampleT, (LEN_PER_CHANNEL + 1) * @as(usize, channels_count));
            var channel_samples: [8][*]SampleT = undefined;
        ...
        pub fn singleChannelIter(self: @This(), channel: u8, len: u16) SingleChannelIter(SampleT) {
        ...
        pub fn wavFill(
            self: *@This(),
            reader: @import("WavReader.zig").BufferedReader.Reader,
            ...
            while (self.len < LEN_PER_CHANNEL) : (self.len += 1) {
                for (0..channels_count) |i| {
                    var sample = reader.readInt(SampleT, .little) catch |err| {
                        switch (err) {
                    ...
                    if (SampleT == i8) {
                        sample = @intCast(@as(i16, @as(u8, @bitCast(sample))) - 128);
                    }
                    self.channel_samples[i][self.start +% self.len] = sample;

                    md5.update(&std.mem.toBytes(std.mem.nativeToLittle(SampleT, sample)));
                }
            }
...
pub fn SingleChannelIter(SampleT: type) type {
    return struct {
        ...
        pub fn sampleIter(self: *@This()) SampleIter(SampleT) {
            return SampleIter(SampleT).init(@This(), self, next, peek, reset, self.len);
        }
...
pub fn FixedResidualIter(SampleT: type) type {
    return struct {
        ...
        prev_samples: @Vector(4, isize),
        order: usize,

        const fp = @import("fixed_prefiction.zig");

        const ResidualRangeError = error{
            OutOfRange, // When residual > maxInt(i32) or <= minInt(i32)
        };
...
pub fn SampleIter(SampleT: type) type {
    return struct {
        ...
        pub fn fixedResidualIter(self: @This(), order: u8) ?FixedResidualIter(SampleT) {
            var prev_samples: @Vector(4, SampleT) = undefined;
            for (1..order + 1) |i| {
